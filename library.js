...
Promise.all([getFiveLetterWords(), getFiveLetterWordsAndHints()]).then(results => {
    wordBank = results[0];
    hints = results[1];
    // Since 'hints' are now available, you can call 'rearrangeHints(hints)' here
    rearrangeHints(hints);
    // Now call 'getRandomWord'
    getRandomWord();
});
async function getFiveLetterWords() {
  const response = await fetch('https://api.datamuse.com/words?sp=?????&md=d&max=1000');
  const data = await response.json();
  const words = data.filter(word => !word.tags || !word.tags.includes('profanity')).map(word => word.word);
  let wordBank = words;
  return wordBank;
}
async function getFiveLetterWordsAndHints() {
    const response = await fetch('https://api.datamuse.com/words?sp=?????&md=d&max=1000');
    const data = await response.json();
    const words = data
        .filter(word => !word.tags || !word.tags.includes('profanity')) // filter out profanity
        .filter(word => word.defs && word.defs.length > 0) // filter out words without definitions
        .map(word => {
            let definition = null;
            for (let def of word.defs) {
                // Remove part of speech and limit length to 13 words
                let tempDef = def.split(' ').slice(1).join(' '); // remove part of speech
                let tempDefWords = tempDef.split(' ');
                if (tempDefWords.length <= 13) {
                    // Replace ')' with ','
                    tempDef = tempDef.replace(/\)/g, ',');
                    definition = tempDef;
                    break;
                }
            }
        ...
        .filter(word => word.definition); // filter out words without definitions (after processing)
    let hintWords = words.reduce((acc, word) => {
      let firstLetter = word.word[0].toLowerCase();
      if (!acc[firstLetter]) {
        acc[firstLetter] = [];
      }
      acc[firstLetter].push({answer: word.word, hint: word.definition});
      return acc;
    }, {});
    return hintWords;
}
